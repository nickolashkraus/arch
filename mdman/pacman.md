...
DESCRIPTION
       ...
       Linux Package Management” library. This library allows alternative front-ends to
       be written (for instance, a GUI front-end).
...
OPERATIONS
       -D, --database
           ...
           attributes of the installed packages in pacman’s database. It also allows
           you to check the databases for internal consistency. See Database Options
           below.
       ...
       -S, --sync
           ...
           provide the same functionality as foo will be searched for. If any package
           is found, it will be installed. A selection prompt is provided if multiple
       ...
       -T, --deptest
           ...
           system. This operation accepts no other options. Example usage: pacman -T qt
           "bash>=3.2".
...
OPTIONS
       -b, --dbpath <path>
           ...
           This should not be used unless you know what you are doing.  NOTE: If
       ...
       -r, --root <path>
           ...
           used as a way to install software into /usr/local instead of /usr.  NOTE: If
           database path or log file are not specified on either the command line or in
           pacman.conf(5), their default location will be inside this root path.  NOTE:
           This option is not suitable for performing operations on a mounted guest
       ...
       --cachedir <dir>
           ...
           they are tried in the order they are passed to pacman.  NOTE: This is an
       ...
       --gpgdir <dir>
           ...
           default is /etc/pacman.d/gnupg). This directory should contain two files:
           pubring.gpg and trustdb.gpg.  pubring.gpg holds the public keys of all
           packagers.  trustdb.gpg contains a so-called trust database, which specifies
           that the keys are authentic and trusted.  NOTE: This is an absolute path,
...
UPGRADE OPTIONS (APPLY TO -S AND -U)
       ...
       --overwrite <glob>
           ...
           Patterns can be negated, such that files matching them will not be
           overwritten, by prefixing them with an exclamation mark. Subsequent matches
           will override previous ones. A leading literal exclamation mark or backslash
           needs to be escaped.
...
QUERY OPTIONS (APPLY TO -Q)
       ...
       -k, --check
           ...
           check all installed packages. Specifying this option twice will perform more
...
SYNC OPTIONS (APPLY TO -S)
       ...
       -s, --search <regexp>
           ...
           descriptions that match regexp. When you include multiple search terms, only
       ...
       -y, --refresh
           ...
           --sysupgrade or -u. Passing two --refresh or -y flags will force a refresh
...
DATABASE OPTIONS (APPLY TO -D)
       ...
       -k, --check
           ...
           file. Specifying this option twice will perform a check on the sync
...
CONFIGURATION
       See pacman.conf(5) for more details on configuring pacman using the pacman.conf
       file.
...
Pacman 5.2.2                           2020-06-25                             PACMAN(8)
