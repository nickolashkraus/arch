...
DESCRIPTION
       ...
       -t {tag}    The file to edit and the initial cursor position depends on a "tag",
                   a sort of goto label.  {tag} is looked up in the tags file, the  as‐
                   sociated file becomes the current file and the associated command is
                   executed.  Mostly this is used for C programs, in which  case  {tag}
                   could  be  a  function name.  The effect is that the file containing
                   that function becomes the current file and the cursor is  positioned
                   on the start of the function.  See ":help tag-commands".

       -q [errorfile]
                   Start  in quickFix mode.  The file [errorfile] is read and the first
                   error is displayed.  If [errorfile] is omitted, the filename is  ob‐
                   tained from the 'errorfile' option (defaults to "AztecC.Err" for the
                   Amiga, "errors.err" on other systems).  Further errors can be jumped
...
OPTIONS
       ...
       -i {viminfo}
                   Specifies the filename to use when reading or  writing  the  viminfo
                   file, instead of the default "~/.viminfo".  This can also be used to
       ...
       -N          No-compatible mode.  Resets the 'compatible' option.  This will make
       ...
       -R          Read-only mode.  The 'readonly' option will be set.  You  can  still
                   ...
                   option (see above).  The 'readonly' option can be reset  with  ":set
       ...
       -u {vimrc}  Use the commands in the file {vimrc} for initializations.   All  the
                   ...
                   the name "NONE".  See ":help initialization" within vim for more de‐
                   tails.
